...
namespace NHST.manager
{
    public partial class Report_recharge : System.Web.UI.Page
    {
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            var la = AdminSendUserWalletController.GetBySQL_DK_WithBankExcel(search, "2", bk, fd, td);
            ...
            foreach (var item in la)
            {
                ...
                StrExport.Append("<td>" + item.Content + "</td>");
            ...
            string strFile = "ExcelReportRecharge.xls";
        ...
        protected void btnExcel1_Click(object sender, EventArgs e)
        {
            ...
            var la = WithdrawController.GetBySQL_DK_newExcel(search, "2", fd, td);
            StringBuilder StrExport = new StringBuilder();
            StrExport.Append(@"<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:excel' xmlns='http://www.w3.org/TR/REC-html40'><head><title>Time</title>");
            StrExport.Append(@"<body lang=EN-US style='mso-element:header' id=h1><span style='mso--code:DATE'></span><div class=Section1>");
            StrExport.Append("<DIV  style='font-size:12px;'>");
            ...
            StrExport.Append("      <th><strong>Trạng thái</strong></th>");
            StrExport.Append("      <th><strong>Ngày tạo</strong></th>");
            StrExport.Append("      <th><strong>Người duyệt</strong></th>");
            StrExport.Append("  </tr>");
            foreach (var item in la)
            {              
                StrExport.Append("  <tr>");
                ...
                StrExport.Append("<td>" + item.StatusName + "</td>");
                StrExport.Append("<td>" + item.CreatedDateString + "</td>");
                StrExport.Append("<td>" + item.AcceptBy + "</td>");               
                StrExport.Append("  </tr>");
            }
            StrExport.Append("</table>");
            StrExport.Append("</div></body></html>");
            string strFile = "ExcelReportWithdraw.xls";
            string strcontentType = "application/vnd.ms-excel";
            Response.ClearContent();
            Response.ClearHeaders();
            Response.BufferOutput = true;
        ...
        public void LoadGrid1()
        {
            ...
            var laWithDraw = WithdrawController.GetBySQL_DK_new(search, "2", pageB, fd, td, 10);
            int totalWithDraw = WithdrawController.GetTotalSQL_new(search, "2", fd, td);
            if (laWithDraw.Count > 0)
            {
                pagingallB(laWithDraw, totalWithDraw);
            }

            double[] data2 = new double[] { totalprice, TotalPriceWithDraw };
            string[] backgroundColor2 = new string[] { "green_cyan_gradient", "red_orange_gradient" };
