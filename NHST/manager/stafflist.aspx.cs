        ...
        public void pagingall(List<View_UserList> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    if (item.RoleID == 6)
                    {
                        var listkhach = AccountController.GetAllSaleID(item.ID);
                        if (listkhach != null)
                        {
                            if (listkhach.Count > 0)
                            {
                                soluongkhach = listkhach.Count;
                            }
                        }                                
                    } 
                    else if (item.RoleID == 3)
                    {
                        var listkhach = AccountController.GetAllDathangID(item.ID);
                        if (listkhach != null)
                        {
                            if (listkhach.Count > 0)
                            {
