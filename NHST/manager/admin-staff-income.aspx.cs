...
namespace NHST.manager
{
    public partial class admin_staff_income : System.Web.UI.Page
    {
        ...
        protected void LoadData()
        {
            ...
            var la = GetAllBySQL(st.ToInt(0), search, fd, td, page, 20);
        ...
        protected void btnPayAll_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(td))
                rTD.Text = td;
            string search = "";
            string stt = Request.QueryString["st"];
            if (!string.IsNullOrEmpty(stt))
                select_by.SelectedIndex = stt.ToInt(0);
            if (!string.IsNullOrEmpty(Request.QueryString["s"]))
            {
                search = Request.QueryString["s"].ToString().Trim();
                search_name.Text = search;
            ...
            history = StaffIncomeController.GetBy_SQLHelper(search, stt.ToInt(0), fd, td);
            if (history.Count > 0)
            {
                            ...
                            if (item.RoleID == 3)
                            {
                                ...
                                if (ac != null)
                                {
                                    if (ac.Status == 2)
                                    {
                                        try
                                        {
                                            double wallet = Math.Round(Convert.ToDouble(ac.Wallet) + Convert.ToDouble(item.TotalPriceReceive), 0);
                                            AccountController.updateWallet(ac.ID, wallet, DateTime.Now, username);
