        ...
        public void LoadData()
        {
            ...
            if (obj_user != null)
            {
                ...
                if (tos.Count > 0)
                {
                    ...
                    double totalall7 = MainOrderController.GetTotalPrice_Thang(UID, 6, "TotalPriceVND", t);
                    double totalall7_deposit = MainOrderController.GetTotalPrice_Thang(UID, 6, "Deposit", t);
                    ...
                    int total = MainOrderController.GetTotalItem(UID, status, fd, td, t, se, typesearch);
            ...
            if (obj_user != null)
            {
                ...
                if (total > 0)
                {
                    ...
                    for (int i = 0; i < acs.Count; i++)
                    {
                        ...
                        html.Append(item.DateShop);
                        html.Append(item.DateTQ);
                        html.Append(item.DateDangVeVN);
                                            ...
                                            if (setNoti.IsSentEmailAdmin == true)
                                            {
                                                ...
                                                if (admins.Count > 0)
                                                {
                                                    foreach (var admin in admins)
                                                    {
                                                        try
                                                        {
                                                            PJUtils.SendMailGmail("gianghuycom@gmail.com.vn.vn.net", "zioghzphiauqbdmw", admin.Email,
                                                                "Thông báo tại GIANGHUY.COM.", "Đơn hàng " + o.ID + " đã được đặt cọc.", "");
                                                        }
                                                        catch { }
                                                    }
                                                }
                                                ...
                                                if (managers.Count > 0)
                                                {
                                                    foreach (var manager in managers)
                                                    {
                                                        try
                                                        {
                                                            PJUtils.SendMailGmail("gianghuycom@gmail.com.vn.vn.net", "zioghzphiauqbdmw", manager.Email,
                                                                "Thông báo tại GIANGHUY.COM.", "Đơn hàng " + o.ID + " đã được đặt cọc.", "");
                                                        }
                                                        catch { }
                                                    }
                                        ...
                                        if (setNoti.IsSentEmailAdmin == true)
                                        {
                                            ...
                                            if (admins.Count > 0)
                                            {
                                                foreach (var admin in admins)
                                                {
                                                    ...
                                                    {
                                                        PJUtils.SendMailGmail("gianghuycom@gmail.com.vn.vn.net", "zioghzphiauqbdmw", admin.Email,
                                                            "Thông báo tại GIANGHUY.COM.", "Đơn hàng " + item.MainOrderID + " đã yêu cầu giao hàng.", "");
                                                    }
                                                    catch { }
                                                }
                                            }
                                    ...
                                    if (setNoti.IsSentEmailAdmin == true)
                                    {
                                        ...
                                        if (admins.Count > 0)
                                        {
                                            foreach (var admin in admins)
                                            {
                                                ...
                                                {
                                                    PJUtils.SendMailGmail("gianghuycom@gmail.com.vn.vn.net", "zioghzphiauqbdmw", admin.Email,
                                                        "Thông báo tại GIANGHUY.COM.", "Đơn hàng " + o.ID + " đã được thanh toán.", "");
                                                }
                                                catch { }
                                            }
                                        }
                        ...
                        if (TienKhachHang >= TongTienPhaiCoc)
                        {
                            foreach (var o in lmain)
                            {
                                if (o.Status == 0 && Convert.ToDouble(o.Deposit) < Convert.ToDouble(o.AmountDeposit) && Convert.ToDouble(o.TotalPriceVND) > 0)
                                {
                                    ...
                                    double custDeposit = Math.Round(amountdeposit - orderdeposited, 0);
                                    if (custDeposit > 0)
                                    {
                                        if (wallet >= custDeposit)
                                        {
                                            ...
                                            MainOrderController.UpdateDeposit(o.ID, obj_user.ID, amountdeposit.ToString());
                                            HistoryPayWalletController.Insert(obj_user.ID, obj_user.Username, o.ID, custDeposit, obj_user.Username + " đã đặt cọc đơn hàng: " + o.ID + ".", walletleft, 1, 1, currentDate, obj_user.Username);
                                            PayOrderHistoryController.Insert(o.ID, obj_user.ID, 2, custDeposit, 2, currentDate, obj_user.Username);
                                            ...
                                            //            foreach (var admin in admins)
                                            //            {
                                            //                NotificationsController.Inser(admin.ID, admin.Username, o.ID, "Đơn hàng " + o.ID + " đã được đặt cọc.",
                                            //                1, currentDate, obj_user.Username, false);
                                            //                string strPathAndQuery = Request.Url.PathAndQuery;
                                            //                string strUrl = Request.Url.AbsoluteUri.Replace(strPathAndQuery, "/");
                                            //                string datalink = "" + strUrl + "manager/OrderDetail/" + o.ID;
        ...
        protected void btnPayAll_Click(object sender, EventArgs e)
        {
            ...
            if (obj_user != null)
            {
                ...
                if (obj_user.Wallet > 0)
                {
                    ...
                    if (lmain.Count > 0)
                    {
                        double TongTienVND = Math.Round(MainOrderController.GetTotalPrice(obj_user.ID, 7, "TotalPriceVND", t), 0);
                        double TongTienDaTra = Math.Round(MainOrderController.GetTotalPrice(obj_user.ID, 7, "Deposit", t), 0);
                        double TienLuuKho = Math.Round(MainOrderController.GetTotalPrice(obj_user.ID, 7, "FeeInWareHouse", t), 0);
                        double TongTienPhaiTra = Math.Round((TongTienVND + TienLuuKho) - TongTienDaTra, 0);
                        double TienKhachHang = Math.Round(Convert.ToDouble(obj_user.Wallet), 0);

                        if (TienKhachHang >= TongTienPhaiTra)
                        {
                            foreach (var o in lmain)
                            {
                                double wallet = 0;
                                ...
                                double moneyleft = Math.Round(totalPay - deposit, 0);
                                double walletLeft = Math.Round(wallet - moneyleft, 0);     
                                ...
                                if (moneyleft > 0)
                                {
                                    if (wallet >= moneyleft)
                                    {
                                        ...
                                        MainOrderController.UpdateDeposit(o.ID, obj_user.ID, totalPay.ToString());
                        ...
                        {
                            PJUtils.ShowMessageBoxSwAlert("Tài khoản không đủ tiền để thanh toán tất cả.", "i", true, Page);
                        }
                    }    
                }
                else
