...
namespace NHST.Controllers
{
    public class StaffIncomeController
    {
        ...
        public static double GetTotalPriceAdmin(int status, string PriceType, string s, string fd, string td)
        {
            ...
            sql += "where mo.Status > 9 AND CONVERT(float,st.TotalPriceReceive) > 0 AND st.Status = " + status + " ";
        ...
        public static List<StaffInCome> GetAllBySQL_Excel(int status, string s, string fd, string td)
        {
            ...
            sql += "from tbl_StaffIncome st ";
            sql += " left outer join(select * from tbl_MainOder) as mo ON st.MainOrderID = mo.ID ";
            sql += "where mo.Status = 10 and Convert(float, st.TotalPriceReceive) >0 and st.RoleID in (3,6) ";
            if (status == 1)
                sql += "and st.Status=1 ";
            if (status == 2)
                sql += "and st.Status=2 ";
        ...
        public static List<tbl_StaffIncome> GetBy_SQLHelper(string s, int Status, string fd, string td)
        {
        ...
        public static int GetTotalWithUID(int UID, int status, string s, string fd, string td)
        {
            ...
            sql += "where CONVERT(float,TotalPriceReceive)>0 and RoleID in (3,6) ";
            //sql += " left outer join(select * from tbl_MainOder) as mo ON st.MainOrderID = mo.ID ";
            //sql += " where mo.Status = 10 and st.UID=" + UID + " and CONVERT(float,st.TotalPriceReceive) > 0 ";
            if (status == 1)
                sql += "and Status=1 ";
            if (status == 2)
                sql += "and Status=2 ";
