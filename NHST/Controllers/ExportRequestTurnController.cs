...
namespace NHST.Controllers
{
    public class ExportRequestTurnController
    {
        ...
        public static string InsertWithUID(int UID, string Username, int MainOrderID, DateTime DateExport, double TotalPriceVND, double TotalPriceCYN,
           double TotalWeight, string Note, int ShippingTypeInVNID, DateTime CreatedDate, string CreatedBy, int TotalPackage, int Status, double FeeInsurrane)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_ExportRequestTurn p = new tbl_ExportRequestTurn();
        ...
        public static string UpdateWithUID(int ID, int UID, string Username, int MainOrderID, DateTime DateExport, double TotalPriceVND, double TotalPriceCYN,
           double TotalWeight, string Note, int ShippingTypeInVNID, DateTime CreatedDate, string CreatedBy, int TotalPackage, int Status, double FeeInsurrane)
        {
            using (var dbe = new NHSTEntities())
            {
                var p = dbe.tbl_ExportRequestTurn.Where(o => o.ID == ID).FirstOrDefault();
                if (p != null)
                {
                    p.UID = UID;
                    p.Username = Username;
        ...
        public static string InsertNew(int UID, string Username, int MainOrderID, DateTime DateExport, double TotalPriceVND, double TotalPriceCYN,
           double TotalWeight, string Note, int ShippingTypeInVNID, DateTime CreatedDate, string CreatedBy, int TotalPackage, int Status, double FeeInsurrane)
        {
            using (var dbe = new NHSTEntities())
            {
        ...
        public static List<tbl_ExportRequestTurn> GetAllByID(int ID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_ExportRequestTurn> pages = new List<tbl_ExportRequestTurn>();
                pages = dbe.tbl_ExportRequestTurn.Where(p => p.ID == ID).ToList();
                return pages;
            }
        }
        ...
        public static int GetTotalIncome(string searchname, int stt, string fd, string td)
        {
            var sql = @"SELECT COUNT(*) as Total ";
            sql += "from tbl_ExportRequestTurn as ext ";
            sql += "left outer join tbl_Account as ac ON ext.UID = ac.ID ";
            sql += "left outer join tbl_Account as sale ON sale.ID = ac.SaleID ";
            sql += "where ext.StatusExport > 1 AND ac.SaleID > 0 ";
            if (stt > 0)
                sql += " AND ext.Status=" + stt;
            if (!string.IsNullOrEmpty(searchname))
            {
                sql += " AND sale.Username Like N'%" + searchname + "%' ";
            }
            if (!string.IsNullOrEmpty(fd))
            {
                var df = DateTime.ParseExact(fd, "dd/MM/yyyy HH:mm", null);
                sql += " AND ext.OutStockDate >= CONVERT(VARCHAR(24),'" + df + "',113)";
            }
            if (!string.IsNullOrEmpty(td))
            {
                var dt = DateTime.ParseExact(td, "dd/MM/yyyy HH:mm", null);
                sql += " AND ext.OutStockDate <= CONVERT(VARCHAR(24),'" + dt + "',113)";
            }
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            int total = 0;
            while (reader.Read())
        ...
        public static List<ExportRequest> GetIncomeSaleTrans(string searchname, int stt, string fd, string td, int pageIndex, int pageSize)
        {
            var list = new List<ExportRequest>();
            var sql = @"SELECT ext.ID as ID, ext.UID as UID, ext.Username as Username, ext.TotalPriceVND as TotalPriceVND, ext.Status as Status, ext.StatusExport as StatusExport,
                       ext.CreatedDate as CreatedDate, ext.DateExport as DateExport, ext.OutStockDate, ext.TotalWeight as TotalWeight, ext.FeeInsurrane as FeeInsurrane, sale.Username as SaleName ";
            sql += "from tbl_ExportRequestTurn as ext ";
            sql += "left outer join tbl_Account as ac ON ext.UID = ac.ID ";
            sql += "left outer join tbl_Account as sale ON sale.ID = ac.SaleID ";
            sql += "where ext.StatusExport > 1 AND ac.SaleID > 0 ";
            ...
            sql += "Order By ext.ID desc OFFSET (" + pageIndex + " * " + pageSize + ") ROWS FETCH NEXT " + pageSize + " ROWS ONLY";
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            int i = 1;
            while (reader.Read())
            {
                ...
                if (reader["DateExport"] != DBNull.Value)
                    ...
                    entity.DateExport = string.Empty;
                ...
                {
                    entity.TotalPriceVND = Convert.ToDouble(reader["TotalPriceVND"]);
                    TotalPriceVND = Convert.ToDouble(reader["TotalPriceVND"]); ;
                }
                if (reader["TotalWeight"] != DBNull.Value)
                {
                    entity.TotalWeight = Convert.ToDouble(reader["TotalWeight"]);
                }
                if (reader["FeeInsurrane"] != DBNull.Value)
                {
                    entity.FeeInsurrance = Convert.ToDouble(reader["FeeInsurrane"]);
                }
                ...
                if (Config != null)
                {
                    Percent = Convert.ToDouble(Config.SalePercentTrans);
                }
                entity.TotalIncome = 0;

                if (TotalPriceVND > 0)
                {
                    TotalReceive = Math.Round(TotalPriceVND * Percent / 100, 0);
                    entity.TotalIncome = TotalReceive;
                }
        ...
        public static List<ExportRequest> GetIncomeSaleTransIncome(string searchname, int stt, string fd, string td)
        {
            var list = new List<ExportRequest>();
            var sql = @"SELECT ext.ID as ID, ext.UID as UID, ext.Username as Username, ext.TotalPriceVND as TotalPriceVND, ext.Status as Status, ext.StatusExport as StatusExport,
                       ext.CreatedDate as CreatedDate, ext.DateExport as DateExport, ext.OutStockDate, ext.TotalWeight as TotalWeight, ext.FeeInsurrane as FeeInsurrane, sale.Username as SaleName ";
        ...
        {
            var list = new List<ExportRequest>();
            var sql = @"SELECT ShippingTypeInVNID,st.ShippingTypeVNName,ert.FeeInsurrane,ert.* FROM dbo.tbl_ExportRequestTurn as ert LEFT OUTER JOIN dbo.tbl_ShippingTypeVN AS st ON ert.ShippingTypeInVNID = st.ID where MainOrderID = 0 and ert.ID = ert.ID and Username like N'%" + searchName + "%'  ";
            if (Status > 0)
                sql += " AND ert.Status =" + Status;
        ...
        {
            var sql = @"SELECT  count(*) as Total FROM dbo.tbl_ExportRequestTurn as ert 
                    LEFT OUTER JOIN dbo.tbl_ShippingTypeVN AS st ON ert.ShippingTypeInVNID = st.ID 
                    where MainOrderID = 0 and ert.ID = ert.ID and Username like N'%" + searchName + "%' ";
            if (Status > 0)
                sql += " AND ert.Status =" + Status;
