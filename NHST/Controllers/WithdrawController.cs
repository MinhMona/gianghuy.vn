...
namespace NHST.Controllers
{
    ...
    {        
        ...
        public static string UpdateAccept(int ID, DateTime ModifiedDate, string AcceptBy)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_Withdraw.Where(f => f.ID == ID).FirstOrDefault();
        ...
        public static int GetTotalSQL_new(string username, string st, string fd, string td)
        {
        ...
        public static List<ListWithdrawNew> GetBySQL_DK_new(string username, string st, int pageIndex, string fd, string td, int pageSize)
        {
            var sql = @"select ID, UID, Username, Amount, Status, CreatedDate, AcceptBy, AcceptDate, Beneficiary, BankNumber, BankAddress, Note ";
            sql += "from tbl_Withdraw ";
            sql += "where Type = 2 And  Username LIKE '%" + username + "%' ";
            if (!string.IsNullOrEmpty(st))
            {
            ...
            while (reader.Read())
            {
                ...
                if (reader["Note"] != DBNull.Value)
                    entity.Content = reader["Note"].ToString();
        ...
        public static List<ListWithdrawNew> GetBySQL_DK_newExcel(string username, string st, string fd, string td)
        {
            var sql = @"select ID, UID, Username, Amount, Status, CreatedDate, AcceptBy, AcceptDate, Beneficiary, BankNumber, BankAddress, Note ";
            sql += "from tbl_Withdraw ";
            sql += "where Type = 2 And  Username LIKE '%" + username + "%' ";
            ...
            while (reader.Read())
            {
                ...
                {
                    entity.Amount = Convert.ToDouble(reader["Amount"].ToString());
                    entity.AmountString = string.Format("{0:N0}", Convert.ToDouble(reader["Amount"].ToString()));
                }
