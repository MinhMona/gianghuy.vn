...
namespace NHST.Controllers
{
    public class AccountController
    {
        ...
        public static List<tbl_Account> GetAllByRoleIDStatus(int RoleID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_Account> las = new List<tbl_Account>();
        ...
        public static string updateTransactionValue(int ID, double value, DateTime ModifiedDate, string ModifiedBy)
        {
            using (var dbe = new NHSTEntities())
            {
                ...
                if (a != null)
                {
                    ...
                    if (a.RoleID == 1)
                    {
                        decimal TotalPrice = MainOrderController.GetTotalPriceVIP(a.ID, "Deposit");
                        if (TotalPrice > 0)
                        {
                            var userLevel = UserLevelController.GetByFromTo(TotalPrice);
                            if (userLevel != null)
                                a.LevelID = userLevel.ID;                            
                        }
                        a.TotalTransactionValue = TotalPrice;
        ...
        public static List<tbl_Account> GetAllDathangID(int s)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_Account> las = new List<tbl_Account>();
                las = dbe.tbl_Account.Where(a => a.DathangID == s).OrderByDescending(a => a.SaleID).ToList();
                if (las.Count > 0)
                {
                    return las;
                }
        ...
        public static List<tbl_Account> GetAll_RoleID_All(string s)
        {
            ...
            {
                List<tbl_Account> las = new List<tbl_Account>();
                las = dbe.tbl_Account.Where(a => a.Username.Contains(s)).OrderByDescending(a => a.ID).ThenByDescending(a => a.CreatedDate).ToList();
                return las;
            }
        }


        public static List<tbl_Account> GetAll_RoleIDSale_All(string s, int RoleID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_Account> las = new List<tbl_Account>();
                las = dbe.tbl_Account.Where(a => a.Username.Contains(s) && a.RoleID == RoleID).OrderByDescending(a => a.ID).ThenByDescending(a => a.CreatedDate).ToList();
                return las;
            }
        }
        public static List<tbl_Account> GetAll_RoleID(int RoleID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_Account> las = new List<tbl_Account>();
        ...
        public static tbl_Account UpdateShippingType(int ID, int shippingType)
        {
            using (var db = new NHSTEntities())
            {
                tbl_Account acc = db.tbl_Account.Where(n => n.ID == ID).FirstOrDefault();
        ...
        public static int GetTotalUser_Thang(string searchName)
        {
            var sql = @"select Total=COUNT(*) ";
            sql += "from View_UserList ";
            sql += " where  RoleID=1  and ( Username like N'%" + searchName + "%' or ID like N'%" + searchName + "%' ";
        ...
        public static List<View_UserList> GetListUserBySQL_Thang(string searchname, int pageIndex, int pageSize)
        {
            var sql = @"select* ";
            sql += "from View_UserList ";
