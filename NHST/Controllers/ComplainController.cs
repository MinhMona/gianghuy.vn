    ...
    public class ComplainController
    {
        ...
        public static List<tbl_Complain> GetAllBySQL(string s, int pageIndex, int pageSize, string fd, string td, int Status)
        {
            ...
            while (reader.Read())
            {
                ...
                if (reader["Amount"] != DBNull.Value)
                {
                    entity.Amount = Convert.ToDouble(reader["Amount"].ToString()).ToString() + " VND";
        ...
        public static List<tbl_Complain> GetAllBySQLExcel(string s, string fd, string td, int Status)
        {
            var sql = @"select * ";
            sql += "from tbl_Complain ";
            sql += "Where CreatedBy LIKE N'%" + s + "%' ";
