...
namespace NHST.Controllers
{
    public class MainOrderController
    {
        ...
        public static string UpdateIsHHForSale(int ID, bool IsHHForSale, DateTime ModifiedDate, int ModifiedBy)
        {
            using (var dbe = new NHSTEntities())
            {
                var or = dbe.tbl_MainOder.Where(o => o.ID == ID).FirstOrDefault();
        ...
        public static string UpdateDateShop(int ID, DateTime DepositDate)
        {
            using (var dbe = new NHSTEntities())
            {
                var or = dbe.tbl_MainOder.Where(o => o.ID == ID).FirstOrDefault();
                if (or != null)
                {
                    or.DateShopPhatHang = DepositDate;
                    dbe.Configuration.ValidateOnSaveEnabled = false;
                    dbe.SaveChanges();
                    return "ok";
                }
        ...
        public static string UpdateDateDangVeVN(int ID, DateTime DepositDate)
        {
            using (var dbe = new NHSTEntities())
            {
                var or = dbe.tbl_MainOder.Where(o => o.ID == ID).FirstOrDefault();
        ...
        public static string UpdateFee_PhuPhiKhac(int ID, string Deposit, string FeeShipCN, string FeeBuyPro, string FeeWeight,
         string IsCheckProductPrice, string IsPackedPrice, string IsFastDeliveryPrice, string TotalPriceVND, string PhuPhiKhac)
        {
            using (var dbe = new NHSTEntities())
            {
                var or = dbe.tbl_MainOder.Where(o => o.ID == ID).FirstOrDefault();
        ...
        public class mainorder
        {
            ...
            public string DateDangVeVN { get; set; }
        ...
        public class OrderGetSQL
        {
            ...
            public string CompleteDate { get; set; }
            public string DateShop { get; set; }
            public string DateVeVN { get; set; }
        ...
        public static int GetTotalUID(int UID, int status, string fd, string td, int OrderType)
        {
        ...
        public static List<mainorder> GetAllByUIDNotHidden_SqlHelperNew(int UID, string search, int typesearch, int status, string fd, string td, int OrderType, int page)
        {
            ...
            if (typesearch != 0)
            {
                if (!string.IsNullOrEmpty(search))
                {
                    ...
                    if (typesearch == 2)
                    {
                        sql += " AND ShopName like N'%" + search + "%'";
                    }
            ...
            sql += " Order By ID desc OFFSET (" + page + "*20) ROWS FETCH NEXT 20 ROWS ONLY";
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            while (reader.Read())
            {
                var entity = new mainorder();
                ...
                if (reader["LinkImage"] != DBNull.Value)
                    entity.anhsanpham = reader["LinkImage"].ToString();
                ...
                if (reader["DateShopPhatHang"] != DBNull.Value)
                    entity.DateShop = "<p class=\"s-txt no-wrap\">Shop phát hàng: " + string.Format("{0:HH:mm}", Convert.ToDateTime(reader["DateShopPhatHang"].ToString())) + " - " + string.Format("{0:dd/MM/yyyy}", Convert.ToDateTime(reader["DateShopPhatHang"].ToString())) + " </p>";
                ...
                if (reader["DateDangVeVN"] != DBNull.Value)
        ...
        public static int GetTotalItem(int UID, int status, string fd, string td, int OrderType, string search, int typesearch)
        {
            var sql = @"select Count(*) as Total from tbl_MainOder";
            sql += " where UID = " + UID + " And OrderType= " + OrderType + " ";
        ...
        public static decimal GetTotalPriceVIP(int UID, string PriceType)
        {
            var sql = @"select total=SUM(CAST(" + PriceType + " as float)) ";
            sql += "from tbl_MainOder ";
            sql += "where UID=" + UID.ToString() + " AND STATUS !=1 AND STATUS != 0 ";
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            decimal total = 0;
            while (reader.Read())
            {
        ...
        public static double GetTotalPrice_Thang(int UID, int status, string PriceType, int orderType)
        {
            var sql = @"select total=SUM(CAST(" + PriceType + " as float)) ";
            sql += "from tbl_MainOder ";
            sql += "where UID=" + UID.ToString() + " AND STATUS > " + status.ToString() + " And OrderType=" + orderType.ToString();
        ...
        public static int GetTotalForOrderListOfDK(int RoleID, int StaffID, int orderType, string searchtext, int Type, string fd, string td, string priceFrom, string priceTo, string st, bool isNotCode, string mvd, string mdh, bool isOrder)
        {
            int a = 0;
            var sql = @"select Total=Count(*) ";
            sql += "FROM    dbo.tbl_MainOder AS mo LEFT OUTER JOIN ";
            ...
            if (!string.IsNullOrEmpty(searchtext))
            {
                ...
                if (Type == 2)
                    sql += " AND EXISTS (SELECT ID FROM dbo.tbl_SmallPackage WHERE OrderTransactionCode like N'%" + searchtext + "%' AND MainOrderID = mo.ID)";
            ...
            {
                sql += " AND EXISTS (SELECT ID FROM dbo.tbl_SmallPackage WHERE OrderTransactionCode like N'%" + mvd + "%' AND MainOrderID = mo.ID)";
            }
            if (!string.IsNullOrEmpty(mdh))
            {
                sql += " AND EXISTS (SELECT ID FROM dbo.tbl_MainOrderCode WHERE MainOrderCode LIKE N'%" + mdh + "%' AND MainOrderID = mo.ID)";
            }
            if (!string.IsNullOrEmpty(fd))
            {
                var df = DateTime.ParseExact(fd, "dd/MM/yyyy HH:mm", null);
        ...
        public static List<OrderGetSQL> GetByUserIDInSQLHelperWithFilterOrderList(int RoleID, int StaffID, int OrderType, string searchtext, int Type, string fd, string td, string priceFrom, string priceTo, string st, bool isNotCode, int pageIndex, int pageSize, string mvd, string mdh, int sort, bool isOrder)
        {
            var list = new List<OrderGetSQL>();
            var sql = @"SELECT mo.ID, mo.TotalPriceVND, mo.CurrentCNYVN, mo.DathangID, mo.SalerID ,mo.IsDoneSmallPackage,sz.MaDonHangString,se.MaVanDon,sw.MaDonHang,sp.TenSanPham, mo.Deposit, mo.CreatedDate, mo.DepostiDate, mo.DateBuy, mo.DateTQ, mo.DateVN,mo.PayDate,mo.CompleteDate, mo.Status, mo.OrderType, ";
            sql += "mo.SalerID, u.Username AS Uname, s.Username AS saler, d.Username AS dathang, sm.totalSmallPackages,mo.IsCheckNotiPrice, mo.DathangID, mo.DateShopPhatHang, mo.DateDangVeVN, ";
            ...
            while (reader.Read())
            {
                ...
                {
                    if (reader["DateShopPhatHang"] != DBNull.Value)
                        entity.DateShop = "<p class=\"s-txt no-wrap red-text\"><span class=\"mg\">Shop phát hàng:</span><span> " + string.Format("{0:HH:mm}", Convert.ToDateTime(reader["DateShopPhatHang"].ToString())) + " - " + string.Format("{0:dd/MM/yyyy}", Convert.ToDateTime(reader["DateShopPhatHang"].ToString())) + "</span> </p>";
                }
                else
                {
                    if (reader["DateShopPhatHang"] != DBNull.Value)
                        entity.DateShop = "<p class=\"s-txt no-wrap\"><span class=\"mg\">Shop phát hàng:</span><span> " + string.Format("{0:HH:mm}", Convert.ToDateTime(reader["DateShopPhatHang"].ToString())) + " - " + string.Format("{0:dd/MM/yyyy}", Convert.ToDateTime(reader["DateShopPhatHang"].ToString())) + "</span> </p>";
                }
                ...
                {
                    if (reader["DateDangVeVN"] != DBNull.Value)
                        entity.DateVeVN = "<p class=\"s-txt no-wrap red-text\"><span class=\"mg\">Hàng đang về VN:</span><span> " + string.Format("{0:HH:mm}", Convert.ToDateTime(reader["DateDangVeVN"].ToString())) + " - " + string.Format("{0:dd/MM/yyyy}", Convert.ToDateTime(reader["DateDangVeVN"].ToString())) + "</span> </p>";
                }
                else
                {
                    if (reader["DateDangVeVN"] != DBNull.Value)
                        entity.DateVeVN = "<p class=\"s-txt no-wrap\"><span class=\"mg\">Hàng đang về VN:</span><span> " + string.Format("{0:HH:mm}", Convert.ToDateTime(reader["DateDangVeVN"].ToString())) + " - " + string.Format("{0:dd/MM/yyyy}", Convert.ToDateTime(reader["DateDangVeVN"].ToString())) + "</span> </p>";
                }
        ...
        public static List<OrderGetSQL> GetByUserIDInSQLHelperWithFilterNew(int UID, string searchtext, int Type, string fd, string td, string priceFrom, string priceTo, string st, bool isNotCode, int pageIndex, int pageSize)
        {
            ...
            sql += "WHEN 4 THEN N'<span class=\"badge orange darken-2 white-text border-radius-2\">Shop phát hàng</span>' ";
            ...
            sql += "WHEN 8 THEN N'<span class=\"badge yellow darken-2 white-text border-radius-2\">Hàng đang về VN</span>' ";
        ...
        public class IncomSaler
        {
            ...
            public int DathangID { get; set; }
            public string TotalPriceVND { get; set; }
            public string PhuPhiKhac { get; set; }
        ...
        public static List<IncomSaler> GetFromDateToDate_IncomSaler_Thang_total(string fd, string td, string Username, string Saler)
        {
            ...
            sql += ", Sum(convert(float,mo.PhuPhiKhac)) as PhuPhiKhac ";
            ...
            sql += ", Sum( (convert(float,mo.PriceVND)) + (convert(float,mo.FeeShipCN)) - (convert(float,mo.TotalPriceReal)) - (convert(float,mo.PhuPhiKhac)) ) as tienmacca ";
            ...
            sql += " where mo.SalerID = ac.ID and mo.UID = kh.ID  and mo.Status >=2 ";
            ...
            while (reader.Read())
            {
                ...
                if (reader["PhuPhiKhac"] != DBNull.Value)
                    entity.PhuPhiKhac = reader["PhuPhiKhac"].ToString();
        ...
        public static List<OrderGetSQL> GetAllOrderList(int RoleID, int StaffID, int OrderType, string searchtext, int Type, string fd, string td, string priceFrom, string priceTo, string st, bool isNotCode, int pageIndex, int pageSize, string mvd, string mdh, int sort, bool isOrder)
        {
            var list = new List<OrderGetSQL>();
            ...
            if (RoleID == 3)
            {
                sql += "    AND M.Status != 1 AND M.DathangID = " + StaffID + "";
            }
            else if (RoleID == 6)
            {
                sql += "    AND M.Status != 1 AND M.SalerID = " + StaffID + "";
            }
            else if (RoleID == 7)
            {
                sql += "    AND M.Status != 1";
            ...
            while (reader.Read())
            {
                ...
                if (reader["Site"] != DBNull.Value)
                    entity.ShopName = reader["Site"].ToString();
                ...
                {
                    if (reader["DateDangVeVN"] != DBNull.Value)
                        entity.DateVN = "<p class=\"s-txt no-wrap red-text\"><span class=\"mg\">Hàng đang về VN:</span><span> " + string.Format("{0:HH:mm}", Convert.ToDateTime(reader["DateDangVeVN"].ToString())) + " - " + string.Format("{0:dd/MM/yyyy}", Convert.ToDateTime(reader["DateDangVeVN"].ToString())) + "</span> </p>";
                }
                else
                {
                    if (reader["DateDangVeVN"] != DBNull.Value)
