...
namespace NHST.Controllers
{
    public class OrderController
    {
        ...
        public static string UpdateLinkIMG(int ID, string image_origin)
        {
            using (var dbe = new NHSTEntities())
            {
        ...
        public static List<tbl_Order> getListOrderImagebase64()
        {
            var list = new List<tbl_Order>();
            var sql = @"SELECT [ID] ";
            sql += " from tbl_Order";
            sql += " where image_origin like '%base64%' ";
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            while (reader.Read())
            {
                var entity = new tbl_Order();
