...
namespace NHST.Controllers
{
    public class RequestOutStockController
    {
        ...
        public static tbl_RequestOutStock GetByTransportOrderID(int id)
        {
            using (var dbe = new NHSTEntities())
            {
                var pages = dbe.tbl_RequestOutStock.Where(p => p.TransportationID == id).FirstOrDefault();
                return pages;
            }
        }
        public static bool CheckIsInRequestOutStock(int smallpackageID)
        {
            using (var dbe = new NHSTEntities())
            {
                var pages = dbe.tbl_RequestOutStock.Where(p => p.SmallPackageID == smallpackageID).FirstOrDefault();
