        ...
        public void pagingall(List<tbl_PayHelp> acs)
        {
            ...
            if (acs.Count > 0)
            {
                ...
                for (int i = FromRow; i < ToRow + 1; i++)
                {
                    ...
                    if (status != 4)
                        html.Append("<a href=\"/them-khieu-nai-thanh-toan-ho/" + item.ID + "\" data-position=\"top\"><i class=\"material-icons\">report</i><span>Khiếu nại</span></a>");
                    if (status == 0)
        ...
        protected void btnPayment_Click(object sender, EventArgs e)
        {
            ...
            if (id > 0)
            {
                    ...
                    if (p != null)
                    {
                        ...
                        if (walletCYN > 0)
                        {
                            if (walletCYN >= TotalPrice)
                            {
                                ...
                                #endregion
                                int st = TransactionController.PayMent(UID, id, walletCYN_left, username, TotalPrice, 1, 1, username + " đã trả tiền thanh toán tiền hộ.", currentDate, 1);
                                if (st == 1)
                                {
                                    if (setNoti != null)
                                    {
                ...
                8, currentDate, username, false);
                                                    ...
                                                    PJUtils.PushNotiDesktop(a.ID, username + " đã trả tiền thanh toán hộ.", datalink);
                                                }
                                            }
                                        }
                                                ...
                                                foreach (var admin in admins)
                                                {
                                                    try
                                                    {
                                                        PJUtils.SendMailGmail("gianghuycom@gmail.com.vn", "zioghzphiauqbdmw", admin.Email,
                                                            "Thông báo tại GIANGHUY.COM.", username + " đã trả tiền thanh toán tiền hộ.", "");
                                                    }
                                                    catch { }
                    ...
                    8, currentDate, username, false);
                        ...
                        else
                        {
                            if (wallet >= TotalPriceVND)
                            {
                                ...
                                int st = TransactionController.PayhelpWallet(id, 1, UID, username, walletleft, TotalPriceVND, username + " đã trả tiền thanh toán tiền hộ.", 1, 9, currentDate);
                                if (st == 1)
                                {
                                    if (setNoti != null)
                                    {
